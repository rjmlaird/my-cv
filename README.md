...
array in the API response. The field is optional — omit it and the tag list
just doesn't render for that item. Unrecognised tags (a name that doesn't
match a slug in `src/content/skills/`) still render as plain text, so the
site never breaks while tags are being backfilled.
